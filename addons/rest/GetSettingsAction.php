...
class GetSettingsAction extends Action {
    ...
    protected function prepareDataProvider() {
        ...
        if ($showbehaviors) {
            ...
            $containers = \app\models\EthogramContainer::findAll(['user_id' => \Yii::$app->user->identity->user_id]);
            $behaviorsGrid = array();
            foreach ($containers as $box) {
                $elements = \app\models\EthogramElements::findAll(['container_id' => $box->container_id]);
                foreach ($elements as $individualB) {
                    $behaviorsGrid[$box->container_name][] = [
                        'name' => $individualB->element_name,
                        'linkable' => null !== $individualB->recepient && (bool) $individualB->recepient ? true : false];
                }
            }

//            Will just keep it here for the safety reasons
...
//            }
            ...
            $setting['appkey']['bhvtest']['recipients'] = ['Self', 'Observer', 'Other'];
        }
